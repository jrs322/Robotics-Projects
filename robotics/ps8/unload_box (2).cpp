...
int main(int argc, char** argv) {
    ...
    while(boxInspector.get_bad_part_Q1(current_part)) {
        ...
        cin>>ans;
	for (int i = 0; i < nparts; i++) { 
          double x_diff = abs(current_part.pose.pose.position.x - orphan_models_wrt_world[i].pose.position.x); 
	  double y_diff = abs(current_part.pose.pose.position.y - orphan_models_wrt_world[i].pose.position.y);
	  if (x_diff <.01 && y_diff <.01) {
