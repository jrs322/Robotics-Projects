//Jrs322 robot gear pusher thing
...
int main(int argc, char** argv) {
    ...
    double gear_goal_x = .45;
    double gear_goal_y = 0.65;
    ...
    int nsteps = 50; //will need to specify how many interpolation points in Cartesian path
    ...
    int attempts = 0;
    double xOffset = abs(g_perceived_object_pose.pose.position.x - gear_goal_x + .01);
    double yOffset = abs(g_perceived_object_pose.pose.position.y - gear_goal_y + .01);

    while (attempts < 3 || (xOffset > 0.08 && yOffset > 0.08)) {
      //xxxxxxxxxxxxxx  the following makes an inquiry for the pose of the part of interest
      //specify the part name, send it in the goal message, wait for and interpret the result
      object_finder_goal.object_name = g_object_name.c_str(); //convert string object to old C-style string data
      object_finder_ac.sendGoal(object_finder_goal,&objectFinderDoneCb); //request object finding via action server
      ...
      if (gear_goal_x - abs(g_perceived_object_pose.pose.position.x) > 0) {
        flange_origin << g_perceived_object_pose.pose.position.x - 0.125, g_perceived_object_pose.pose.position.y, 0.75;
      }
      ...
      if (abs(gear_goal_x - g_perceived_object_pose.pose.position.x) > 0) {
       flange_origin << g_perceived_object_pose.pose.position.x - 0.125, g_perceived_object_pose.pose.position.y, 0.75;
      }
      else {
        flange_origin << g_perceived_object_pose.pose.position.x + 0.125, g_perceived_object_pose.pose.position.y, 0.75;
      ...
      if (gear_goal_x - abs(g_perceived_object_pose.pose.position.x) > 0) {
        flange_origin << g_perceived_object_pose.pose.position.x + 0.1, g_perceived_object_pose.pose.position.y, 0.75;
      }
      else {
        flange_origin << g_perceived_object_pose.pose.position.x - 0.1, g_perceived_object_pose.pose.position.y, 0.75;
      }
      ...
      ROS_INFO("done with last trajectory");
          ...
          goal_flange_affine.linear() = R_down;
          if (gear_goal_y - g_perceived_object_pose.pose.position.y > 0) {
            flange_origin << g_perceived_object_pose.pose.position.x, g_perceived_object_pose.pose.position.y - 0.1, 0.75;
          }
          else {
            flange_origin << g_perceived_object_pose.pose.position.x, g_perceived_object_pose.pose.position.y + 0.1, 0.75;
          }
          ...
          //xxxxxxxxxxxxxxxxxx
           ...
           //manually prescribed flange pose; in the  future, compute this based  on perception
           if (gear_goal_y - abs(g_perceived_object_pose.pose.position.y) > 0) {
             flange_origin << g_perceived_object_pose.pose.position.x, g_perceived_object_pose.pose.position.y - 0.1, 0.75;
           }
           else {
             flange_origin << g_perceived_object_pose.pose.position.x, g_perceived_object_pose.pose.position.y + 0.1, 0.75;
          ...
          finished_before_timeout = object_finder_ac.waitForResult(ros::Duration(15.0)); //wait for a max time for response
          //NOTE: could do something else here (if useful) while waiting for response from action server
          if (!finished_before_timeout) {
                  ROS_WARN("giving up waiting on result "); //this should not happen; should get result of found or not-found
                  return 1;
          ...
          xOffset = abs(g_perceived_object_pose.pose.position.x - gear_goal_x +.1);
          yOffset = abs(g_perceived_object_pose.pose.position.y - gear_goal_y +.1);
          attempts++;
    }
}
