...
public:
    ...
    : it_(nh_) {
        ...
        block_pose_publisher_ = nh_.advertise<geometry_msgs::PoseStamped>("bp", 1, true);
...
void ImageConverter::imageCb(const sensor_msgs::ImageConstPtr& msg){
        ...
        double x_normal = (x_centroid - 320) * scale;
        double y_normal = (y_centroid - 240) * scale;
        double theta = .206;
        double x = cos(theta) * x_normal - sin(theta) * y_normal + .543;
        double y =(sin(theta) * x_normal) + cos(theta) * y_normal - .320;
        block_pose_.pose.position.x =  x;
        block_pose_.pose.position.y =  -1 * y;

        ROS_INFO("u is %f", x_normal);
        ROS_INFO("v is %f", y_normal);
	ROS_INFO("x is %f", block_pose_.pose.position.x); 
	ROS_INFO("y is %f", block_pose_.pose.position.y); 
